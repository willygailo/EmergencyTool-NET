...
export const AssignmentScreen = ({ route, navigation }: any) => {
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    try {
      await api.put(`/responder/assignments/${assignmentId || '1'}/status`, { status: newStatus });
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.mapFallbackCard}>
          <Text style={styles.fallbackTitle}>Map Preview (Web Fallback)</Text>
...
const styles = StyleSheet.create({
  ...
  mapFallbackCard: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12 },
  fallbackTitle: { fontSize: 14, fontWeight: '600', color: '#64748b', marginBottom: 8 },
  navigateButton: {
    marginTop: 12,
  ...
  coords: { fontSize: 12, color: '#666', marginTop: 4, fontFamily: 'monospace' },
